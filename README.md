...
2. **Facilio CMMS connector** (`vibe.executeAction('facilio-cmms', …)` → `/api/runtime/connections/*`) — the org's real data via list actions: `list-sites`/`list-buildings`/`list-floors` (**the portfolio source**), `list-spaces` (per-floor units), `list-employees` (**people directory**), and `list-assets` (**Edit-mode asset catalog**). Enabled via `VITE_USE_CONNECTORS` (on in `.env.production`). The connector exposes no booking/assignment write actions, so those still fall through; `list-work-orders` / `list-tenants` are available for future features.
3. **This app's own Vibe DB** (`vibe.executeFunction('floorplanApi', ...)` → `/api/runtime/functions/*`) — units/assignments/bookings/employees (the app-specific data the connector doesn't wire), once `vibe db create` + a deployed `floorplanApi` function exist. Throws until that's set up. **Does not serve the portfolio** — that comes from the CMMS connector above, not a KV blob copied into this app (`VibeDbDataSource.getPortfolio` throws to stay out of the portfolio union).
   ...
   **Mirror cache**: every connector read (tier 2) flows through `mirrorThroughCache` (`lib/moduleCache.ts`), which keeps a per-module copy in the vibe-db (`getCache`/`putCache` handlers, `app_data` under a `cache:` prefix) **equal to the connector API — rewriting it only when the data changed** — and serves that copy when the connector is unreachable. It degrades to a plain pass-through when the `floorplanApi` function isn't reachable (e.g. `npm run dev`). Requires redeploying `floorplanApi` (for the new handlers) to activate.
