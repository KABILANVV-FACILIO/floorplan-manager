...
const banner = `// GENERATED by scripts/gen-rcu-data.mjs — do not edit by hand.
// Source: RCU-Corporate Occupancy export (${rows.length} rows, ${skipped} skipped).
// Desk/room geometry is synthetic (no coordinates in the export): spaces are
// packed as room clusters per floor with desks gridded inside.
`;

const out = `${banner}
import type { Assignments, Employee, Site, Unit } from './types';

export const RCU_PORTFOLIO: Site[] = ${JSON.stringify(sites, null, 2)};

export const RCU_EMPLOYEES: Employee[] = ${JSON.stringify([...employeesById.values()], null, 2)};

export const RCU_UNITS: Unit[] = ${JSON.stringify(units, null, 2)};

export const RCU_ASSIGNMENTS: Assignments = ${JSON.stringify(assignments, null, 2)};
`;

writeFileSync(new URL('../src/lib/rcuData.ts', import.meta.url), out);
